...
class AdapterProductComputer(private val productList: MutableList<Product>) :
    RecyclerView.Adapter<AdapterProductComputer.ViewHolder>() {
        ...
        init {
            element.setOnClickListener {
                ...
                it.findNavController().navigate(
                    ProductsComputerFragmentDirections.actionProductsComputerFragmentToProductFragment(
                        "${name.text}",
                        "${prodId.text}",
                        "${imageName.text}",
                        "${reference.text}",
